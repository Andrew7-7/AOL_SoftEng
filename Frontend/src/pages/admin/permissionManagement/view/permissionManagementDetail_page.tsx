  ...
  const fetchPermissionData = async () => {
    try {
      const response = await axios.get(
        `http://localhost:3002/permission/getPermission/${permissionId}`
      );
  ...
  const openImageInNewWindow = (imageUrl: string) => {
    ...
    if (newWindow) {
      newWindow.document.write(
        `<img src="${imageUrl}" alt="Certification Image" style="width:100%;">`
      );
        ...
        <div className="permission-management-detail-page-center">
          <div className="permission-management-detail-content-container">
            <div className="permission-management-detail-header-section">
              <h1 className="permission-management-detail-header">
                Permission Management Detail
              </h1>
              <Link
                to="/permissionManagement"
                className="permission-management-detail-back-link"
              >
                Back
              </Link>
            </div>
            <div className="permission-management-detail-details-section">
              <div className="permission-management-detail-item">
                <span className="permission-management-detail-title">
                  Permission ID:
                </span>
                <span className="permission-management-detail-content">
                  {reqCourse?.permissionID}
                </span>
              </div>
              <div className="permission-management-detail-item">
                <span className="permission-management-detail-title">
              ...
              <div className="permission-management-detail-item-certification">
                ...
                <div className="permission-management-detail-content-image-container">
                  <img
                    ...
                    alt="Certification"
                    onClick={() =>
                      openImageInNewWindow(reqCourse?.certificateImg!)
                    }
                    style={{ cursor: "pointer" }}
                  />
                </div>
                <span className="permission-management-detail-info">
                  Please Click to see the full size of the picture
                </span>
              </div>
              {status !== "Accepted" && status !== "Denied" && (
                <div className="permission-management-detail-button-container">
                  <button
                    className="permission-management-detail-action-button accept"
                    onClick={handleAccept}
                  >
                    Accept
                  </button>
                  <button
                    className="permission-management-detail-action-button deny"
                    onClick={handleDeny}
                  >
                    Deny
                  </button>
